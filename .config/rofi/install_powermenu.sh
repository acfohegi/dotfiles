...
SOURCE_SCRIPT="$XDG_CONFIG_HOME/rofi/powermenu.sh"

BIN_DIR="$HOME/.local/bin"
BIN_PATH="$BIN_DIR/powermenu"

DESKTOP_DIR="$XDG_DATA_HOME/applications"
DESKTOP_FILE="$DESKTOP_DIR/powermenu.desktop"

# Ensure source script exists
if [ ! -f "$SOURCE_SCRIPT" ]; then
  echo "powermenu.sh not found"
  exit 1
fi
...
Type=Application
Name=Power Menu
GenericName=Power and login controls
Exec=powermenu
Icon=system-shutdown
Categories=System;
EOF

echo "Power menu installed successfully."
